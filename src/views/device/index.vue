  ...
  <page-header-wrapper>
    <a-card :bordered="false">
              ...
              <a-col :md="8" :sm="24" v-show="itemIndex < 2 || advanced">
                <a-form-item :label="item.searchTitle || item.title">
                  ...
                  <a-select v-if="item.searchType == 2" v-model="queryParam[item.dataIndex]" placeholder="">
                    <a-select-option
                      v-for="(target, index) in item.searchMap.length > 0 ? item.searchMap : 2"
                      :key="index"
                      :value="target.value"
                    >
                      <span v-if="item.searchMap.length > 0">{{ target.name }}</span>
                      <span v-else>{{ item.title + '--' + value }}</span>
                    </a-select-option>
                  </a-select>
                  <!-- 时间选择器 -->
                  <a-date-picker
                    v-else-if="item.searchType == 3"
      ...
      <s-table
        ...
        :scroll="{ x: 1600, y: 500 }"
      >
        <span slot="online" slot-scope="text" :class="'status-color' + text">
        ...
        <span slot="action" slot-scope="text, record">
          <template>
            <a @click="handleAddOrEdit(record)">更新</a>
            <a-divider type="vertical" />
            <a @click="handleDel(record)">删除</a>
            <a-divider type="vertical" />
            <a @click="handleToggleDetail(record)" disabled>详情</a>
          </template>
        </span>
      </s-table>
      <create-form
      ...
      />
      <!-- 机器详情 -->
      <HostDetailDrawer :visible="detailVisible" :model="detailInfo" @drawClose="handleToggleDetail(null)">
      </HostDetailDrawer>
    </a-card>
  </page-header-wrapper>
</template>
...
import { getHostList, addHost, updateHost, deleteHost } from '@/api/device'
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          ...
          if (type === 2) {
            ...
            newParam.id = values.id
            newParam.sn = values.sn
            newParam.uid = values.uid
            newParam.online = values.online
            newParam.net_type = values.net_type
            newParam.ipStack = values.ip_Stack
            values = newParam
          }
    ...
    handleItemOper(type, param) {
      ...
      funArr[type]
        ...
        .catch((err) => {
          console.log(err)
          this.confirmLoading = false
          this.$message.error(`${funArr[type].title}失败`)
        })
    },
    // 表格多选
